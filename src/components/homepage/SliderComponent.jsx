...
function SliderComponent() {
  ...
  const totalWidth = figureWidth * figuresCount;
  ...
  return (
    ...
    <div className="grid items-center relative mx-auto w-full max-w-[75%] overflow-hidden p-10 3xl:w-[2026px] 3xl:mx-auto">
      {isLoading ? (
        <motion.div
        ...
        >
          ...
          {/* Repeat the same skeletons to create the loop effect */}
          {items.map((_, index) => (
            <figure
        ...
        >
          {items.map((item, index) => (
            <figure
              ...
              className="flex flex-col sm:flex-row justify-center items-center gap-2"
            >
              <img src={item.imgSrc} alt={item.alt} className="w-[50px] sm:w-[70px]" />
              <figcaption className="text-2xl sm:text-4xl flex flex-col sm:items-center sm:justify-center font-bold text-primary">
                {item.caption}
              </figcaption>
            </figure>
          ))}
          {/* Repeat the same items to create the loop effect */}
          {items.map((item, index) => (
            <figure
              key={index + figuresCount}
              className="flex flex-col sm:flex-row justify-center items-center gap-2"
